...
function normalizeContext(context = {}) {
  ...
  const margen = totalVentas > 0 ? toNumber(context.margen ?? (utilidadBruta / totalVentas * 100)) : 0;
...
function allZero(ctx) {
  return ctx.totalVentas === 0 && ctx.totalCompras === 0;
}
...
function negativeUtilityMonths(ctx) {
  return monthlyUtilities(ctx)
    .map((value, index) => ({ value, month: MONTHS[index] }))
    .filter((item) => item.value < 0);
}

function resultStatus(ctx) {
  if (ctx.utilidadBruta < 0) return 'negativo';
  if (ctx.margen < 15) return 'bajo';
  if (ctx.margen < 30) return 'moderado';
  return 'saludable';
}

function analyzeDashboard(ctx) {
  if (allZero(ctx)) {
    return 'Aún no hay datos registrados para este año. Para obtener un análisis, ingresa tus ventas y compras en **Registro de Datos**. Te recomiendo comenzar con enero o con el periodo que tengas disponible.';
  }
  ...
  const lines = [
    `En el año ${ctx.year} tienes:`,
    `1. Ventas totales: **${money(ctx.totalVentas, ctx.currency)}**.`,
    `2. Compras totales: **${money(ctx.totalCompras, ctx.currency)}**.`,
    `3. Utilidad bruta: **${money(ctx.utilidadBruta, ctx.currency)}**.`,
    `4. Margen: **${ctx.margen.toFixed(2)}%**.`,
    '',
    status === 'negativo'
      ? 'Tus compras superan tus ventas en el periodo analizado. Revisa los meses con mayor compra y evalúa si corresponden a inversión, inventario o exceso de costos.'
      : status === 'bajo'
        ? 'El margen es bajo. Podrías revisar precios, costos de compra o meses con menor rentabilidad.'
        : status === 'moderado'
          ? 'El resultado es positivo, pero todavía hay espacio para mejorar el margen revisando costos y meses con menor rendimiento.'
          : 'El margen es positivo. Puedes usar este reporte para comparar si este comportamiento se mantiene en los siguientes meses.',
    '',
  ...
  if (negativeMonths.length) {
    lines.push(`Meses con utilidad negativa: **${negativeMonths.map((item) => item.month).join(', ')}**.`);
  }
...
function replyForIntent(message, ctx) {
  ...
  if (has('que hago primero', 'primer paso', 'empezar', 'inicio', 'como empiezo', 'nuevo usuario')) {
    return 'Primero ve a **Registro de Datos**. Ingresa las ventas y compras de cada mes. Luego revisa la **Vista General** para ver tus totales, utilidad y margen. Finalmente usa **Guardar reporte** para consultarlo después.';
  }

  if (has('para que sirve', 'que es anualprint', 'anualprint')) {
    return 'AnualPrint sirve para registrar ventas y compras mensuales, calcular utilidad bruta y margen, guardar reportes por usuario y generar un informe anual imprimible para revisión financiera.';
  }
  ...
  if (has('compras', 'total compras', 'compra')) {
    return 'Las **compras** representan los costos o adquisiciones registradas por mes. AnualPrint las compara con ventas para calcular utilidad bruta y margen.';
  }
  ...
  if (has('renombrar', 'editar nombre', 'cambiar nombre')) {
    return 'Para renombrar un reporte, entra a **Mis reportes** y usa **Editar nombre**. Esto solo cambia el título; no modifica ventas, compras ni cálculos.';
  }
  ...
  if (has('datos en cero', 'cero', 'sin datos', 'no hay datos', 'no hay operaciones')) {
    return 'Aún no hay datos registrados. Te recomiendo comenzar ingresando ventas y compras del mes de enero o del periodo que tengas disponible en **Registro de Datos**.';
  }

  if (has('comparativo mensual', 'comparar meses', 'tendencia', 'distribucion', 'distribución')) {
    return 'El comparativo mensual permite ver cómo se comportan ventas y compras mes a mes. La tendencia ayuda a identificar crecimiento o caída. La distribución anual muestra el peso de ventas, compras y utilidad en el periodo.';
  }
  ...
  return 'Puedo ayudarte principalmente con el uso de AnualPrint, registro de datos, reportes, ventas, compras, utilidad, margen y análisis financiero básico dentro de la aplicación.';
}
